...
const makeDefaultBubbles = (queries, isBigScreen) => {
    ...
    const queriesCopy = queries.slice(
        0,
        isBigScreen
            ? config.defaultBubblesCount
            : config.defaultBubblesCountMobile
    );
    ...
    const res = queriesCopy.map((s, i) =>
        makeDefaultBubble(s, sizes[i], max, isBigScreen)
    );
    res.push({
...
const makeDefaultBubble = (s, size, max, isBigScreen) => {
    ...
    return {
        ...
        color: size,
        style: {
            color: lightness > 0.65 ? "#333" : "white",
            fontSize: `${(0.65 + size / max) * 100 * (isBigScreen ? 1 : 0.7)}%`,
            animation: `pulse ${4 + Math.random() * 10}s ease-in-out infinite`,
            animationDelay: `-${Math.random() * 10}s`,
            animationDirection: "alternate",
            backgroundColor: `hsl(${Math.round(230 + Math.random() * 5)}, ${
                (0.25 + Math.random() * 0.4) * 100
            }%, ${lightness * 100}%)`,
        },
    };
};
    ...
    return (
        <>
            ...
            <Chart
                ...
                viewPersonQuotes={viewPersonQuotes}
            />
        </>
    );
});
...
const Chart = React.memo(function ({ query, setLoading }) {
    ...
    const defaults = useMemo(
        () => makeDefaultBubbles(defaultBubbles, isBigScreen),
        [isBigScreen]
    );
        ...
        (async () => {
            ...
            try {
                ...
                if (!res.length) {
                    setError(
                        <div>
                            אין תוצאות :(&nbsp;&nbsp; נסו ללחוץ על אחת הבועות?
                        </div>
                    );
                    setData(defaults);
                    return;
                ...
                const minRatio = isBigScreen
                    ? minRatioForImage
                    : minRatioForImageSmallScreen;
                const total = res
                    .map((r) => r.Counter)
                    .reduce((a, b) => a + b, 0);
                ...
                const filteredRes = res
                    ...
                    .slice(
                        0,
                        isBigScreen ? undefined : maxBubblesForSmallScreen
                    );
    ...
    return (
        <>
            ...
            <Snackbar message={error} open={!!error} style={{ zIndex: 25 }} />
                ...
                <Treemap
                    ...
                    onLeafClick={(n) => {
                        if (n.data.result) {
                            ...
                            else setPersonPreview(n.data.result);
                        } else if (n.data.query) navigate({ q: n.data.query });
...
const MobilePersonCard = React.memo(function ({
    PersonID,
    FirstName,
    LastName,
    Text,
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        {(FactionName || KnessetNum) && (
                            <Typography
                                ...
                                color="textSecondary"
                            >
                                {FactionName && `${FactionName}, `}הכנסת ה-
                                {KnessetNum}
                            </Typography>
                        )}
                    </div>
...
const PersonCard = React.memo(function ({
    PersonID,
    FirstName,
    LastName,
    Text,
...
const SuggestionBubble = React.memo(function ({ query }) {
    return (
        <div className="suggestion">
            <div style={{ textAlign: "center", width: "100%" }}>{query}</div>
        </div>
    );
});
...
function getDimensions(windowSize) {
    ...
    return Math.max(
        minSize,
        Math.min(size, windowSize.height * maxHeightRatio)
    );
}
