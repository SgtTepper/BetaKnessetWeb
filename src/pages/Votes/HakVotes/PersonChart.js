...
const PersonChart = React.memo(function ({setLoading, queryString, minDifference, setMaxDifference, setWorstParty, setBestParty, setBestPartyImg, setWorstPartyImg}) {
    ...
    return (
        <>
            <div  style={{zIndex:'3', position: 'absolute', maxWidth:'100vw', maxHeight: '100vh', placeItems: 'center', float:'left', textAlign:'center'}} className="dynamic-treemap-example" className={classes.root}>
                {/*<DiscreteSlider max={total} minDifference={minDifference} setMinDifference={setMinDifference} />*/}

                <div style={{position:'relative'}}>
                    <div style={{position:"absolute", left:'-570px', top:'-20px', zIndex:'5'}}>
                                <Treemap
                            padding={10}
                            animation={true}
                            data={{
                    ...
                    <div style={{position:"absolute", left:'-970px', top:'110px', zIndex:'4'}}>
                        <Treemap
                            padding={10}
                            animation={true}
                            data={{
        ...
        <div style={{maxHeight: '100%'}}>
            ...
            <div style={{background: 'linear-gradient(45deg, #c5cae9 30%, #e8eaf6 70%)'}}>
                ...
                <table style={{minWidth: '100%'}}>
                    <th style={{ background: 'linear-gradient(45deg,green 20%, #daded8 90%)', color: 'rgba(24,24,53,0.8)'}}> אנחנו מסכימים ({agreed_laws == "" || agreed_laws== undefined ? 0 : agreed_laws.split('#').length}):</th>
                    { agreed_laws == "" || agreed_laws== undefined ? <tr><td>אני ואתה לא באותו ראש                                         <span style={{fontSize: '20px'}} aria-label="sad" role="img"> 😕 </span>
                    </td></tr> :agreed_laws.split('#').map((item)=> <React.Fragment><tr><td>{item} </td></tr></React.Fragment>) }</table>

                <table style={{minWidth: '100%'}}>
                    <th style={{minWidth: '100%' , background: 'linear-gradient(45deg, red 20%, #cfb1b9 90%)', color: 'rgba(24,24,53,0.8)'}}>  אנחנו לא מסכימים ({disagreed_laws == "" || disagreed_laws== undefined ? 0 : disagreed_laws.split('#').length}): </th>
                    {disagreed_laws == "" || disagreed_laws== undefined ? <tr><td>שתי טיפות מים אנחנו                                     <span style={{fontSize: '20px'}} aria-label="blush" role="img"> 😊 </span>
